...
// Added for sensor inferfacing, Lars 2009
...
// Added for sensor inferfacing, Lars 2009
void Player::handleExternalInput() {
	// std::cout << "Hey hey, there I'am!!!" << std::endl;
	ControlEvent controlEvent = TuxControlSingleton::getInstance()->removeControlEvent();
	if (controlEvent.valid)	{
		// std::cout << "Valid key: ";
		int keyState;
		if (controlEvent.event == KEY_RELEASED) {
			keyState = UP;
			// std::cout << "UP ";
		}
		if (controlEvent.event == KEY_PRESSED) {
			keyState = DOWN;
			// std::cout << "DOWN ";
		}
